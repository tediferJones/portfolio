  ...
  return (
    t('div', {
      ...
      onmouseleave: () => {
        ...
        document.querySelector(`#${title}-trigger`)?.classList.remove('bg-blue-700')
      }
    }, [
        t('button', {
          // className: 'text-lg flex gap-2 items-center hover:bg-sky-700 p-4 rounded-xl mr-4 sm:mr-0 transition-colors duration-300',
          className: 'text-lg flex gap-2 items-center hover:bg-blue-700 p-4 rounded-xl mr-4 sm:mr-0 transition-colors duration-300',
          id: `${title}-trigger`,
          onclick: () => {
            ...
            document.querySelector(`#${title}-trigger`)?.classList.add('bg-blue-700')
          }
        }, [
        ...
        t('div', {
          ...
          className: 'hidden h-8 w-8 bg-blue-700 absolute top-14 rotate-45',
          id: `${title}-tail`
        }),
        // t('div', { className: 'hidden bg-sky-700 absolute top-16 px-4 py-2 rounded-xl z-10 right-1 sm:right-auto',
        t('div', { className: 'hidden bg-blue-700 absolute top-16 px-4 py-2 rounded-xl z-10 right-1 sm:right-auto',
          id: `${title}-body`
        }, content.map((key, i) => {
            return (
              t('div', { className: (key.name && contactMenuToggles.includes(key.name) ? 'lg:hidden' : '') }, [
                LinkTo({
                  ...
                  className: 'my-1 p-2 whitespace-nowrap flex justify-center items-center gap-4 hover:bg-blue-800 rounded-xl transition-colors duration-300',
